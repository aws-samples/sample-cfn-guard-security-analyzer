...
for i in 0 1 2 3; do
    ...
    # Container deployment is required because the agents spawn the awslabs MCP
    # servers via stdio (uvx). Direct code deploy ships only Python and would
    # fail at runtime with FileNotFoundError: 'uvx'. The Dockerfile in
    # agents/ pre-installs both MCP servers and uvx itself.
    agentcore configure \
