...
export default function ResultsSection({ analysis }: ResultsSectionProps) {
  ...
  const handleRefresh = () => {
    if (!analysis.resourceUrl) return;
    // Re-run the same analysis with the cache bypass flag. The orchestrator
    // re-invokes AgentCore (or restarts Step Functions) and rewrites the cache
    // entry on success. The "Cached" badge clears via the COMPLETE action.
      ...
      header={
          ...
          actions={
            <SpaceBetween direction="horizontal" size="xs">
              <Button
                ...
                disabled={!analysis.resourceUrl}
                ariaLabel="Refresh analysis (bypass cache)"
              >
                Refresh
              </Button>
              <Button
        ...
        >
          <SpaceBetween direction="horizontal" size="xs">
            ...
            {analysis.cached && (
              <Badge color="grey">{formatCachedLabel(analysis.cachedAt)}</Badge>
            )}
          </SpaceBetween>
        </Header>
      }
