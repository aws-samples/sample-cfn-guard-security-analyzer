...
/**
 * Cache-hit display: when the orchestrator returns `cached: true`, the Results
 * pane renders a "Cached <timestamp>" badge. Validates the formatter used by
 * that badge handles the three cases:
 *   1. valid ISO 8601 timestamp → "Cached <localized date>"
 *   2. malformed timestamp string → bare "Cached" (don't render NaN/Invalid Date)
 *   3. null timestamp (older cache rows) → bare "Cached"
 *
 * Backs the Phase 3 "show a small Cached badge on cached responses" requirement.
 */
describe("formatCachedLabel — cache-hit badge text", () => {
  it("falls back to bare 'Cached' when cachedAt is null", () => {
    expect(formatCachedLabel(null)).toBe("Cached");
  });

  it("falls back to bare 'Cached' when cachedAt is unparseable", () => {
    expect(formatCachedLabel("not a real timestamp")).toBe("Cached");
  });

  it("renders a localized timestamp when cachedAt is valid ISO 8601", () => {
    const iso = "2026-05-23T10:30:00.000Z";
    const label = formatCachedLabel(iso);
    expect(label.startsWith("Cached ")).toBe(true);
    // The exact localized string depends on the test environment's locale,
    // but it must include a recognizable year token from the timestamp.
