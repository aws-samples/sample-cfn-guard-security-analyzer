...
export function routeSSEEvent(
  ...
  switch (event) {
    ...
    case "complete": {
      // Phase 9: backend dual-names totalProperties / totalPropertiesDiscovered
      // for compat. Accept either; prefer the canonical name.
      const d = data as {
        totalProperties?: number;
        totalPropertiesDiscovered?: number;
        resourceType?: string;
      };
      const total = d.totalProperties ?? d.totalPropertiesDiscovered ?? 0;
      options.onComplete(total, d.resourceType);
      break;
    }
    case "error": {
