...
def get_environment_config(environment_name: str = "dev") -> EnvironmentConfig:
    ...
    cfg = ENVIRONMENTS[environment_name]
    # Allow env vars to override the in-file placeholders so users don't have to
    # edit config.py to deploy. CDK_DEFAULT_ACCOUNT/REGION are CDK conventions.
    account = os.environ.get("CDK_DEFAULT_ACCOUNT") or cfg.account
    region = os.environ.get("CDK_DEFAULT_REGION") or cfg.region
    if account == "YOUR_AWS_ACCOUNT_ID":
        raise ValueError(
            "AWS account ID not configured. Set CDK_DEFAULT_ACCOUNT environment variable "
            "or replace YOUR_AWS_ACCOUNT_ID in config.py."
        )
    cfg.account = account
    cfg.region = region
    return cfg
